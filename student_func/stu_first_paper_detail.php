<!-- 学生论文初稿明细文件 -->
<!-- 导师调用该文件查看对应的学生论文初稿摘要明细，并为其上传修改意见，同时可以确定该学生的论文初稿最终版 -->
<!-- 传递参数为 topic_id -->
<?php
include "../link.php";
if (isset($_GET["id"])) {
...
$sql_first_paper = "SELECT * FROM `first_paper_record` WHERE `first_paper_record`.`topic_id` = '{$get}' AND `record_id` = '{$row_id['max(`record_id`)']}'";
...
<body>
        ...
        <table data-toggle="table" data-toolbar="#toolbar">
            <tbody>
                <tr>
                    ...
                    <td class='col-xs-9'>
                        <?php echo $row_first_paper['paper_main']; ?>
                    </td>
    ...
    <?php
    if ($row_first_paper['final_flag'] == 2 || $row_first_paper['final_flag'] == 1) {
        echo "<br/>";
        echo "";
    } else {
        echo "<br/>";
        echo "<a href=\"../tutor_func/t_add_suggestion.php?func=first_paper&cid={$get}\" 
        ...
        <div class="chose-student-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <?php
                    echo "<form action=\"../tutor_func/t_add_suggestion.php?func=first_paper&id={$get}\" method=\"POST\" class=\"form-horizontal\">";
                    ?>
                    <div class="form-group">
                        <label for="inputTopicName" class="col-sm-2 control-label">主要修改意见</label>
                        <div class="col-sm-8">
                            <textarea name="paper_suggestion" class="form-control" rows="20"><?php echo $row_first_paper['modify_suggestion']; ?></textarea>
                        </div>
                    </div>
