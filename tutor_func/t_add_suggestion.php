...
if (isset($_GET["func"])) {
    ...
    switch ($result) {
        ...
        case "final_paper":
            if (isset($_GET['id'])) {
                $sql_id = "SELECT max(`record_id`) from `final_paper_record` WHERE `final_paper_record`.`topic_id` = '{$_GET['id']}' order by `record_id` desc";
                $result_id = mysqli_query($link, $sql_id);
                $row_id = mysqli_fetch_array($result_id, MYSQLI_BOTH);

                $sql = "UPDATE  `final_paper_record` set `modify_suggestion`='{$_POST['paper_suggestion']}',`final_flag` = 2 
                WHERE `final_paper_record`.`topic_id` = '{$_GET['id']}' AND `record_id` = '{$row_id['max(`record_id`)']}'";
                mysqli_query($link, $sql);
                mysqli_close($link);
                echo "<script>alert('上传意见成功！');history.go(-1)</script>";
            }
            if (isset($_GET['cid'])) {
                ...
                $sql_id = "SELECT max(`record_id`) from `final_paper_record` WHERE `final_paper_record`.`topic_id` = '{$_GET['cid']}' order by `record_id` desc";
                $result_id = mysqli_query($link, $sql_id);
                $row_id = mysqli_fetch_array($result_id, MYSQLI_BOTH);


                $sql = "UPDATE  `final_paper_record` set `final_flag` = 1 
                WHERE `final_paper_record`.`topic_id` = '{$_GET['cid']}' AND `record_id` = '{$row_id['max(`record_id`)']}'";
                $sql_2 = "UPDATE  `reply_schedule` set `final_paper_flag` = 1 
                WHERE `reply_schedule`.`id` = '{$row_s['student_id']}' ";
                mysqli_query($link, $sql);
                mysqli_query($link, $sql_2);
                mysqli_close($link);
                echo "<script>alert('已同意此学生的论文终稿！');history.go(-1)</script>";
            }
            break;
    }
}
