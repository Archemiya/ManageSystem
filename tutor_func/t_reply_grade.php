...
function echo_second_reply_schedule_table($link)
{
    ...
    //查询所有二辩学生
    $sql_student_num_2 = "SELECT * FROM `second_reply_schedule`
    where `permission` = 'student' ";
    $result_student_num_2 = mysqli_query($link, $sql_student_num_2);
    $num_student_2 = mysqli_num_rows($result_student_num_2);
    echo <<< archemiya
    <div class=" alert alert-warning" role='alert'>
    二辩评分
    </div>
    <div class="table-responsive">
        <table data-toggle="table" >
            ...
            <tbody>
archemiya;
    for ($i = 0; $i < $num_student_2; $i++) {
        $row_student_2 = mysqli_fetch_array($result_student_num_2, MYSQLI_BOTH);
        echo "<tr>";

        $sql_topic_2 = "SELECT * FROM `topic`WHERE `student_id` = '{$row_student_2['id']}' ";
        $result_topic_2 = mysqli_query($link, $sql_topic_2);
        $row_topic_2 = mysqli_fetch_array($result_topic_2, MYSQLI_BOTH);

        //根据学生id查询学生的答辩记录情况
        $sql_record_2 = "SELECT * FROM `reply_record` WHERE `student_id` = '{$row_student_2['id']}' ";
        $result_record_2 = mysqli_query($link, $sql_record_2);
        $row_record_2 = mysqli_fetch_array($result_record_2, MYSQLI_BOTH);

        //根据学生id查询学生成绩情况(此处查询的为学生成绩记录表)
        $sql_grade_2 = "SELECT * FROM `student_second_reply_grade_record` 
        WHERE `teacher_id` = '{$_SESSION['user_id']}' AND `student_id` = '{$row_student_2['id']}'";
        $result_grade_2 = mysqli_query($link, $sql_grade_2);
        $row_grade_2 = mysqli_fetch_array($result_grade_2, MYSQLI_BOTH);

        //根据学生id查询学生最终成绩情况(此处查询的为学生成绩表)
        $sql_fgrade_2 = "SELECT * FROM `student_grade` 
        WHERE `student_id` = '{$row_student_2['id']}'";
        $result_fgrade_2 = mysqli_query($link, $sql_fgrade_2);
        $row_fgrade_2 = mysqli_fetch_array($result_fgrade_2, MYSQLI_BOTH);

        echo "<td class=\"td-height th-title-center\">";
        echo "<a href='tutor.php?func=reply_grade&id={$row_topic_2['id']} '>" . $row_topic_2['name'] . "</a>";
        echo "</td>";
        echo "<td class=\"td-height th-title-center alert alert-info\" role='alert' >";
        echo   $row_student_2['id'] . $row_student_2['name'];
        echo "</td>";

        if (!isset($row_fgrade_2['second_grade'])) {
            if (isset($row_fgrade_2['reply_record_annex_name'])) {
                echo "<td>";
                echo "<a class='btn btn-primary' role='button'
            data-toggle=\"modal\" data-target=\"#second_{$row_student_2['id']}\">
            二辩评分
            </button>";
                echo "</td>";
                echo "<td>";
                echo "<a href='../uploaded_files/reply_record_files/{$row_student_2['id']}/{$row_record_2['reply_record_annex_name']}' 
            class='btn btn-primary' role='button'>
            下载附件
            </button>";
                echo "</td>";
        ...
        } elseif (($row_fgrade_2['second_grade']) != 0) {
            echo "<td class='th-title-center alert alert-info' role='alert'>";
            echo "最终成绩：";
            echo $row_fgrade_2['second_grade'];
            echo "</td>";
            echo "<td>";
            echo "";
            echo "</td>";
...
function echo_the_student_reply_schedule_table($link)
{
    //查询所有指导学生学生
    $sql_the_student_num = "SELECT * FROM `topic` 
...
archemiya;
    for ($i = 0; $i < $num_the_student; $i++) {
        ...
        //查询学生论文终稿交稿情况
        $sql_the_final = "SELECT * from `final_paper_record` where `final_flag` = 1 AND `student_id` = '{$row_the_student['student_id']}'";
        $result_the_final = mysqli_query($link,$sql_the_final);
        $row_the_final = mysqli_fetch_array($result_the_final,MYSQLI_BOTH);
        $num_the_final = mysqli_num_rows($result_the_final);

        //根据学生id查询学生此项成绩得分情况
        $sql_the_grade = "SELECT * FROM `student_grade` WHERE `student_id` = '{$row_the_student['student_id']}' ";
        $result_the_grade = mysqli_query($link, $sql_the_grade);
        $row_the_grade = mysqli_fetch_array($result_the_grade, MYSQLI_BOTH);

        echo "<td class=\"td-height th-title-center\">";
        echo "<a href='tutor.php?func=reply_grade&id={$row_the_student['id']} '>" . $row_the_student['name'] . "</a>"; //此处显示课题名称
        echo "</td>";
        echo "<td class=\"td-height th-title-center alert alert-info\" role='alert' >";
        echo   $row_the_student['student_id'] . $row_the_student['student_name'];
        echo "</td>";

        if (!isset($row_the_grade['teacher_grade'])) {
            if ($num_the_final) {
                echo "<td>";
                echo "<a class='btn btn-primary' role='button'
            data-toggle=\"modal\" data-target=\"#the_student_{$row_the_student['student_id']}\">
            指导教师评分
            </button>";
                echo "</td>";
                echo "<td>";
                echo "<a href='../uploaded_files/final_paper_files/{$row_the_student['student_id']}_{$row_the_final['record_id']}/{$row_the_final['final_paper_annex_name']}' 
            class='btn btn-primary' role='button'>
            下载附件
            </button>";
                echo "</td>";
...
<body>
    ...
    if ($row_stu_control['final_paper']) {
        echo_the_student_reply_schedule_table($link);
    }
...
archemiya;
    ...
    for ($i = 0; $i < $num_student; $i++) {
        ...
        echo <<< archemiya
    <div class="modal fade " id="the_student_{$row_the_student['student_id']}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="chose-student-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title login-title">论文终稿评阅</h4>
                </div>
                <div class="modal-body">
archemiya;
        echo "<form action=\"../score.php?func=teacher_grade&id={$row_the_student['student_id']}\" method=\"POST\" class=\"form-horizontal\">";
        echo "<input type=\"hidden\" name=\"topic_id\" value=\"{$row_the_student['id']}\">";
        echo "<input type=\"hidden\" name=\"student_id\" value=\"{$row_the_student['student_id']}\">";
        echo "<input type=\"hidden\" name=\"student_name\" value=\"{$row_the_student['student_name']}\">";
...
archemiya;
    ...
    for ($i = 0; $i < $num_student_2; $i++) {
        ...
        $row_topic_2 = mysqli_fetch_array($result_topic_2, MYSQLI_BOTH);
        echo <<< archemiya
     <div class="modal fade " id="second_{$row_student_2['id']}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
         <div class="chose-student-dialog">
             <div class="modal-content">
                 <div class="modal-header">
                     <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                         <span aria-hidden="true">&times;</span></button>
                     <h4 class="modal-title login-title">二辩成绩</h4>
                 </div>
                 <div class="modal-body">
 archemiya;
        echo "<form action=\"../score.php?func=second_reply&id={$row_topic_2['student_id']}\" method=\"POST\" class=\"form-horizontal\">";
        echo "<input type=\"hidden\" name=\"topic_id\" value=\"{$row_topic_2['id']}\">";
        echo "<input type=\"hidden\" name=\"student_id\" value=\"{$row_topic_2['student_id']}\">";
        echo "<input type=\"hidden\" name=\"student_name\" value=\"{$row_topic_2['student_name']}\">";
        echo "<input type=\"hidden\" name=\"teacher_id\" value=\"{$_SESSION['user_id']}\">";
        echo "<input type=\"hidden\" name=\"teacher_name\" value=\"{$_SESSION['user_name']}\">";
